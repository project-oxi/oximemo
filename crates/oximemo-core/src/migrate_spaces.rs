...
/// file (dated note captures) — dotfiles (`.DS_Store` and other OS
/// cruft) exempt: the post-migration container level only ever sees
/// that debris next to the space dirs, and it must not resurrect the
/// flat signature (and flip the status to `MergeRequired`). A visible
/// stray note still triggers.
fn flat_signature(root: &Path) -> bool {
    ...
    for e in entries.flatten() {
        ...
        if name.starts_with('.') {
            continue; // OS cruft, not flat content
        }
        if e.path().is_file() {
            return true;
        }
...
pub fn maybe_migrate(home: &Path) -> Result<FlatMigrationStatus> {
    ...
    for entry in std::fs::read_dir(&flat)?.flatten() {
        ...
        // The destination dir was just created *inside* the flat root;
        // never move it into itself.
        if entry.path() == space {
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn stray_dotfile_does_not_trigger_merge_required() {
        let home = tempfile::tempdir().unwrap().keep();
        seed_flat(&home);
        maybe_migrate(&home).unwrap();
        // Finder drops .DS_Store at the now-container level; OS cruft is
        // not flat content and must not resurrect the flat signature.
        std::fs::write(crate::spaces::spaces_root(&home).join(".DS_Store"), b"junk").unwrap();
        assert!(matches!(
            maybe_migrate(&home).unwrap(),
            FlatMigrationStatus::AlreadyMigrated
        ));
        // A visible stray file next to personal/ still reads as a flat vault.
        std::fs::write(crate::spaces::spaces_root(&home).join("stray.md"), "x").unwrap();
        assert!(matches!(
            maybe_migrate(&home).unwrap(),
            FlatMigrationStatus::MergeRequired { .. }
        ));
    }
    ...
    #[test]
    fn documents_rewrite_handles_trailing_slash_spelling() {
        let home = tempfile::tempdir().unwrap().keep();
        seed_flat(&home);
        let flat = crate::spaces::spaces_root(&home);
        seed_brain_dir(
