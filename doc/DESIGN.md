...
**스키마 인지 (2026-08-24 시행, spec `2026-08-24-copilot-schema-awareness-design.md`).** "지식 노트 만들어줘"·"영화 등록해줘"가 스키마를 이해한 채 수행되도록 vault를 **CLI로 자기서술화**했다(GUI/에이전트 패리티). 네 가지 조각: (1) 컨텍스트 블록에 `folders:` 구조 지도 팩트(경로·노트 수·프리셋·워크스페이스 마커, 상한 64 + `daily_folder`, 스키마 파싱 실패는 `schema_errors:` 팩트로 보고 — 전체 스키마는 주입하지 않는다); (2) CLI `folders`/`schema` — 폴더 열거와 스키마·템플릿 덤프(JSON, 스키마 없음은 `null`이라는 사실); (3) CLI `collection install/list`·`new --set` — 설치와 **원커맨드 스키마 유효 생성**(전이가 생성 시점에 발화); (4) 메타데이터 어댑터를 공유 크레이트 `oximemo-metadata`(동기 ureq, core는 network-free 유지)로 추출해 CLI `metadata search`/`stamp`로 그라운딩(GUI 채우기와 동일 게이트·채우기 전용 계약). 이 과정에서 movie 프리셋 템플릿의 따옴표 없는 `{{date}}`가 YAML 하드 파싱 에러로 템플릿 스탬핑 전체를 죽이는 기존 버그를 발견·수복(회귀 테스트 상비).
