...
### 4.3 컬렉션 관리 pane — 한 곳에서 전부, 컬렉션별 설정 확장

7종(지식·데일리 시스템 + 책·영화·블로그·소설·아이디어 설치형)이 각각 한 행:
아이콘·이름·(미설치 시 한 줄 설명)·확장 chevron(설치된 것만)·**스위치**.
...
**확장 영역 = 컬렉션별 설정 홈** (4차 정리, 사용자 요청 "컬렉션마다 설정이 여러
개일 수 있다"). 컬렉션이 설정을 독립적으로 늘려도 이 영역이 흡수한다 — 레일
탭을 늘리지 않는다:
- **위치 자동 고정 (2026-08-23)**: 설치 = "이 컬렉션을 사이드바에 띄우고 싶다".
  `installCollection` 성공 후 `setFolderPinned(folder, true)` 자동 호출 +
  `["config"]` 무효화. 컬렉션 관리 pane의 스위치를 켜는 즉시 사이드바 위치에
  행이 나타난다. 해제 = `delete_folder`가 core에서 핀 정리(`retain | f.path != path && !f.path.starts_with(&fp)`) — 고스트 핀 없음. 2026-08-23 이전 설치
  볼트 보정: 1회성 마이그레이션 `oximemo.collectionAutopin.v1` (App `CollectionAutopin` 훅)이 메타 프리셋 마커 폴더 중 미고정 항목을 핀 — user report "설치해도
  사이드바에 안 나타남". 마커 기록 후 재실행 안 함 → 이후 사용자의 명시적
  unpin 존중.
