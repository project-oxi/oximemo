...
2026-08-23 2차 정리(사용자 피드백): 외관/동작/고급/정보처럼 내용 몇 줄 안 되는
탭의 파편화를 없애고(12+N → 8+N), 컬렉션 pane이 아닌 기능 설정(데일리 enabled)은
일반으로, 폴더 관리의 무소속 그룹은 볼트로 흡수했다. 메타데이터 키 입력은
행 안 128px password 필드(스크린샷 리뷰에서 "입력 필드가 안 보인다"고 판정됨)에서
2행 카드+풀와이드 입력+👁 토글+"키 구성됨" 배지로 재설계.

### 4.3 컬렉션 패널 — 얇게 유지

각 컬렉션 탭은: 폴더 경로 표시·"폴더로 이동"·제거(폴더 삭제, 기존 `FoldersSection`의 2클릭 확인 패턴 재사용). 이름 변경·고정은 중복 구현하지 않고 "폴더 관리 탭에서" 안내로 링크. 책/영화 탭에는 "프로바이더 설정은 연동 → 메타데이터에서" 링크만 — 키 관리를 컬렉션마다 중복시키지 않는다. 이 얇음은 의도된 것 — 존재하지도 않는 설정을 채우지 않는다.

**데일리/지식 탭은 예외 하나**: "제거"는 설치형과 의미가 다르다 — 시스템 폴더 재생성 의미론(삭제 후 재시작하면 빈 프리셋 재생성, 기존 동작 유지)이 그대로 적용되고, 책/영화 등 설치형의 제거는 영구다. 이 차이를 버튼 라벨/문구로 구분한다(예: 데일리="초기화", 설치형="제거"). 기존 `[daily] enabled` 토글은 처음 데일리 탭에 뒀다가 2차 정리에서 **일반 → 데일리 노트** 섹션으로 이동했다("모든 TOML 필드는 GUI 설정 가능" 0.9.3 원칙은 유지되고, 컬렉션 pane은 관리만 담당).
