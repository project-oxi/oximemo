...
- 날짜: 2026-08-23
- 상태: 구현 완료 (2026-08-23, 커밋 cb4c516 / 1f4e91a / 0ae636e / be16194 / 2743810 / 3cd377a / 2d5c9e1) — 단 §3.5 검색/스탬프 UX(속성 패널 "메타데이터 채우기")와 어댑터 HTTP 실장(fetch_* 스텝)은 후속. i18n 키(metadata_fill_*)는 확보됨.
...
일반
 ├─ 외관        (테마·언어·독 아이콘)
 ├─ 동작        (휴지통 보관 기간 등, 기존 GeneralSection)
 └─ 캡처        (CaptureSection)
...
설치된 컬렉션                    ← 동적, §2.1 마커/폴백으로 계산
 ├─ 데일리
 ├─ 지식
 ├─ (책/영화/블로그/소설/아이디어 — 설치된 것만)
 └─ + 컬렉션 추가
...
**데일리/지식 탭은 예외 두 가지**: ① "제거"는 설치형과 의미가 다르다 — 시스템 폴더 재생성 의미론(삭제 후 재시작하면 빈 프리셋 재생성, 기존 동작 유지)이 그대로 적용되고, 책/영화 등 설치형의 제거는 영구다. 이 차이를 버튼 라벨/문구로 구분한다(예: 데일리="초기화", 설치형="제거"). ② **데일리 탭에는 기존 `[daily] enabled` 토글을 노출한다** — 지금까지 TOML 전용이었던 필드로, "모든 TOML 필드는 GUI 설정 가능"(0.9.3 원칙)의 갭을 이번에 메운다.
