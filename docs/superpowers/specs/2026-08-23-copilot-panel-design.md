...
- 상태 갱신 (2026-08-24 개정 3): **어댑터 3~5호 시행 — Claude Code·Codex CLI·OxiCode (실측 계약, §13).** claude `-p --output-format=json`(stdin 컨텍스트, `-r`, `--model`, 턴별 modelUsage/permission_denials 폭로), codex `exec --json`(stdin `<stdin>` 블록, `exec resume`, `-m`), oxicode `--print --mode=json`(컨텍스트가 프롬프트에 탑재, 세션 id 재개 없음 → 싱글턴). §11(권한 플래그 부착 금지)은 5개 어댑터에서 더 무게를 가진다.
