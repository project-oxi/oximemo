...
- **`peak_status` 시맨틱**: 직전 상태 스냅샷이 아니라 **역사상 최고 상태**다.
  ...
  순서를 쓴다.
- 검증은 **경고 수준**: 필수 누락·허용값 위반은 속성 패널과 카드에 표시하되
...
- **속성 칩바**: 스키마의 select/multiselect 속성 값 칩을 폴더 칩바 옆에 노출,
  클릭으로 필터 토글.
...
`[review]` 블록이 선언된 폴더에서 폴더 화면 상단 탭으로 진입한다.
...
- 여러 폴더가 `[review]`를 선언하면 각 폴더 탭은 그 폴더 범위, 사이드바
  진입점은 전체 통합 목록을 보여준다.
...
스탬프, 지식 프리셋, 속성 패널(자유 모드 → 타입 편집기).
테스트: 검증 경고 산출, 전이 멱등성, `merge = "max"` 보존, `on = "write"`
발화, mtime 리로드.

**3단계 — Knowledge UX**: 배지, 속성 칩바, 정렬, 복습 대기열과 두 액션,
링크 스캔·리네임 전파 확장, 그래프 배지 색, i18n 키(ko/en 동시), Tauri IPC 등록.
...
1. NoteQuery의 CLI 문법 (`--where key=value --sort key` vs 별도 서브커맨드).
2. 속성 칩바와 기존 태그 필터의 관계(병치 vs 통합).
3. 스키마 위반 경고의 표시 강도(카드에도 표시할지, 패널 한정인지) 및 `doctor`
   리포트 편입 여부.
4. 지식 프리셋 도메인 코드표 기본값(원 설계 7+3개 전부 vs 최소 셋).
5. 복습 대기열 사이드바 통합 진입점의 위치(FAVORITES 인접 vs 별도 섹션).
...
- 복습 "설명 가능함" 액션이 전이 발화 조건상 동작 불가 → `on = "write"` 도입(§6.2, §7.3).
- 배지·정렬·대기열이 `status`/프리셋 폴더에 하드코딩 의존 → `badge`,
  `[properties.*.colors]`, `[review]` 선언 기반으로 일반화. 리뷰가 제안한
  `kind = "knowledge"` 마커는 그 자체가 예외이므로 채택하지 않고 기능 선언으로
  대체(§2 기각 목록, §7).
- §3 aliases 자기모순·참조 오타 → aliases를 "코어 예약 관례 키"로 정직하게 명시.
- 템플릿 프론트매터 strip 동작 변경 미기재 → §6.1·§9에 명시.
- `number` 타입과 `PropValue` 불일치 → v1에서 `number` 제거.
- aliases 충돌 해석 규칙·중복 개념 경고 부재 → H1 > aliases, created 우선, 경고(§5.3).
- 코드 배치·층별 테스트 의무 부재 → props.rs/schema.rs 분리 + 단계별 테스트 의무(§11).
- YAGNI(PropDesc, 전체 속성 tantivy 색인) → v1 축소(§5.2, §10).
- 전이 다중 매칭 순서·no-op 저장 → 선언 순서, `on` 기본값 `change`(§6.2).
- 캡처 H1 승격 규칙 부재 → 첫 줄 승격 규칙(§7.4), 대량 시딩은 범위 밖(§10).
- §12 누락 결정 → 자동저장 충돌·외부 편집 비대칭·다중 폴더 범위·SCHEMA 리로드는
  파킹하지 않고 §7.1·§6.2·§7.3에서 **결정**했다. 남은 5건만 §12에 유지.

**수용한 지적** (코드 대조):

- `hash_memo`가 속성을 제외해 sync 누락 → 해시에 속성 포함 + 마이그레이션(§5.1, §9).
- `MemoSummary`/`NoteDto`/`types.ts`에 props 전파 누락 → 명시(§5.1).
- 속성 정렬이 커서 페이지네이션(`by_sort`)과 충돌 → 오프셋 경로 분리 결정(§5.2).
- tantivy 스키마 변경이 `INDEX_FORMAT_VERSION`으로 감지되지 않음 → 검색 인덱스
  버전 마커(§5.2, §9).
- `load_template`이 본문만 반환, `create_note`가 비-blank 본문에 템플릿 미호출 →
  시그니처·분기 변경(§6.1).
- 리네임 전파가 본문만 재작성 → 속성 값 링크도 재작성(§5.3).
- i18n 키·Tauri IPC 등록·브라우저 폴백 범위 → 3단계 작업 항목으로 명시(§7.5, §11).

**기각한 지적**:

- "`write_document` NoOp이 `set_props`에서 항상 Written이 되는 것은 BLOCKER" →
  값이 실제로 바뀌면 쓰기가 발생하는 것이 정상이다. 같은 값 재설정은 의미 비교로
  NoOp이 유지되므로 자동저장도 안전하다. 결함이 아니라 계약이므로 §5.1에 명문화만
  했다.
