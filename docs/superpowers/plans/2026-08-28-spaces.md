...
- **(Amended 2026-08-29, brain 0.10 cutover plan)** Brain registration follows
  the cutover model: `ensure_document_root` + the desktop index trigger. No
  `register_vault` / recorder / `sync_run` anywhere; no `brain_list_spaces`.
...
```rust
    ...
    pub fn open_spec(spec: &crate::spaces::VaultSpec) -> Result<Self> {
        ...
        // Brain documents-plane glue (2026-08-29 cutover, done in
        // mainline): the space derives from the vault directory name —
        // a space IS its directory, so configured space keys no longer
        // exist. `ensure_document_root` is a pure filesystem no-op
        // gate; ingestion is the desktop's single-flight
        // `oxibrain admin index --documents` trigger.
...
**Already landed in mainline by the cutover plan (2026-08-29 Task 1/2) —
these steps are DONE, listed only for standalone readers:** the
`resolve_space` deletion, the `BrainConfig` socket/space → executable
rework with stale-key tolerance, the recorder-based registration tests
replaced by documents.toml assertions, and the desktop
`BrainEndpointConf`/`brain_connect` replacement by `spawn_brain`
(caller-owned `oxibrain admin serve --stdio` child) with
`vault_space_name` at the command sites. Tasks below that referenced
`register_vault`, `brain_list_spaces`, or `Registration` recorders now
read against the cutover surface instead.
...
**Files:**
- Modify: `apps/desktop/src-tauri/src/lib.rs` (boot resolution), `lib.rs` commands module (three commands + `SpaceInfo`), `generate_handler` list — (`brain_list_spaces` removal already landed in the 2026-08-29 cutover)
...
Register in `generate_handler`: `commands::space_list, commands::space_create, commands::space_switch,`. (`brain_list_spaces` was already removed by the 2026-08-29 cutover.)
...
- Modify: `apps/desktop/src/lib/api.ts` (space fns), `apps/desktop/src/lib/types.ts` (add space config key if needed), `apps/desktop/src/lib/tauri.ts` (fallbacks for the three space commands), `apps/desktop/src/lib/paletteCommands.ts` (two commands), `apps/desktop/src/stores/ui.ts` (picker state), `apps/desktop/src/components/SettingsMenu.tsx` (Brain section derived-space readout), `apps/desktop/src/lib/locales/ko.ts` + `en.ts` (keys) — (`brainListSpaces`/`BrainSpaces`/the `brain_list_spaces` fallback/`socket` in brain config already removed by the 2026-08-29 cutover)
...
(Done in the 2026-08-29 cutover: `brainListSpaces`/`BrainSpaces` removed from
`api.ts`; the brain config type in `types.ts` is `{ enabled?, executable? }`;
the `brain_list_spaces` fallback case removed from `tauri.ts`.) Add the three
space-command fallbacks:
...
`SettingsMenu.tsx` `BrainSection` — already rewritten by the 2026-08-29 cutover
to `enabled` + `executable` rows. Add only the derived read-only space row:

```tsx
      <div className="rounded-lg bg-surface-sunken px-3 py-2">
```

(The row reads the active space from the `space_list` IPC above; the brain
section no longer has socket/space config rows.)
