...
> **실행 모델:** `zai/glm-5.2`
> **기준 CWD:** `/Volumes/MERCURY/PROJECTS/oxinot` (작업 중 oxipage, oxios도 참조)
> **의뢰인:** 사용자
> **사용 스킬:** `design-farmer`
...
3개의 프로젝트(oxinot, oxipage, oxios)의 디자인 시스템을 분석하여 **하나의 일관된 DESIGN.md**를 정립하라.

## 스타일 선호도 (중요)

| 순위 | 프로젝트 | 비고 |
|---|---|---|
| **1위 (가장 선호)** | **oxinot** | 가장 마음에 드는 스타일. 메인 레퍼런스로 삼을 것 |
| 2위 | oxipage | 다음으로 선호 |
| 3위 | oxios | 가장 덜 선호하지만, 좋은 요소는 참고 |

> oxinot도 아직 디자인이 완비되지 않아 허점이 많다. 완벽하지 않다는 전제로 접근하고, 부족한 부분은 design-farmer가 채워나갈 것.
...
| 프로젝트 | 경로 | 기존 디자인 문서 |
|---|---|---|
| **oxinot** | `/Volumes/MERCURY/PROJECTS/oxinot` | `doc/DESIGN.md` (788줄, v0.2, 가장 최근) |
...
- **프로젝트 유형:** 3개의 독립된 프론트엔드 프로젝트 (oxinot: Tauri 2 + React 19, oxipage: Vite + React 19, oxios: Vite + React 19)
- **패키지 매니저:** bun
- **CSS 접근법:** Tailwind v4 + CSS 변수
- **색상 시스템:** OKLCH 기반 (oxinot, oxipage 둘 다 OKLCH 사용. oxios는 OKLCH 아닐 수 있음)
- **디자인 성숙도:** EMERGING (서로 다른 디자인 시스템, 통합 필요)
- **기존 DESIGN.md:** oxinot `doc/DESIGN.md` (가장 상세), oxipage `doc/03-design-system.md` (중간), oxios `web/DESIGN.md` + `DESIGN.md` (여러 개)
- **프레임워크:** 모두 React 19
- **아이콘:** lucide-react (oxinot, oxipage, oxios 일부)
- **언어:** TypeScript
...
1. **Scope (3개 프로젝트 통합):** oxinot의 스타일을 메인 레퍼런스로, oxipage와 oxios의 요소를 통합한다.
2. **Headless or not:** 필요에 따라 oxinot은 Headless UI + Tailwind, oxipage/oxios는 Radix UI 사용 중. 통합 디자인은 Tailwind v4 + CSS 변수 기반으로.
3. **Theme support:** 3개 프로젝트 모두 다크/라이트 테마 지원 필요.
4. **Typography:** oxinot의 Pretendard 폰트 시스템을 기본으로.
5. **Spacing scale:** 4px 그리드 기반.
6. **Border radius:** oxinot의 rounded-xl (12px) 카드 + rounded-lg (8px) 버튼 스타일을 기본으로.
7. **Elevation/shadow:** oxinot의 subtle shadow 스타일.
8. **Design output location:** 각 프로젝트의 기존 디자인 문서 위치에 업데이트. 메인 unified DESIGN.md는 `oxinot/doc/UNIFIED-DESIGN.md`에 저장하고, 각 프로젝트에도 사본/참조를 추가.
...
### 11.1 oxinot (Tauri macOS app)
### 11.2 oxipage (SSG web)
### 11.3 oxios (web app)
```

**저장 위치:**
1. `oxinot/doc/UNIFIED-DESIGN.md` — 메인 통합 문서
2. `oxinot/doc/DESIGN.md` — 기존 DESIGN.md 업데이트 (통합 시스템 반영)
...
- [ ] `oxinot/doc/UNIFIED-DESIGN.md` — OKLCH 토큰, semantic tokens, component tokens 포함
- [ ] `oxinot/doc/DESIGN.md` 업데이트
...
## 수행한 작업
- design-farmer: 3개 프로젝트(oxinot, oxipage, oxios) 디자인 분석
- OKLCH 색상 시스템 통합
- 통합 DESIGN.md (UNIFIED-DESIGN.md) 생성
- 각 프로젝트별 adaptation 문서화

## 산출물
- oxinot/doc/UNIFIED-DESIGN.md
...
- oxinot 스타일을 기본 레퍼런스로, oxipage/oxios는 거기에 맞춰 통합
